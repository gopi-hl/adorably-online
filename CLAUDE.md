...
### File Structure (flat, no src directory)
```
├── App.tsx              # Main app component with routing, filters, theme toggle
...
│   ├── PromptDetail.tsx # Full detail view with MockPageContext wrapper
...
**Prompt Data Flow**: `constants.ts` defines prompts → `App.tsx` filters/selects → `PromptCard` displays → `PromptDetail` shows full view
